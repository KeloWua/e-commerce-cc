    ...
    return (
        <div className="min-h-screen bg-gray-50/50 dark:bg-gray-950 py-12 transition-colors duration-300">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* User Info Header */}
                <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-indigo-500/10 mb-8 overflow-hidden relative transition-all duration-300">
                    ...
                    <div className="relative flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-2">{user.name}</h1>
                        ...
                        <button onClick={handleLogout} className="px-6 py-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 font-bold rounded-2xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-all flex items-center gap-2">
                ...
                {!activeSection ? (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {mainOptions.map((option, i) => (
                                option.link ? (
                                    <Link key={i} to={option.link} className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-indigo-500/10 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-500/30 transition-all group flex flex-col gap-4">
                                        <div className="w-12 h-12 flex items-center justify-center bg-gray-50 dark:bg-gray-800 rounded-2xl text-gray-400 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/50 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                            <option.icon className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-gray-900 dark:text-white text-lg mb-1">{option.title}</h3>
                                            <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">{option.desc}</p>
                                        </div>
                                    </Link>
                                ) : (
                                    <button key={i} onClick={option.action} className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-indigo-500/10 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-500/30 text-left transition-all group flex flex-col gap-4">
                                        <div className="w-12 h-12 flex items-center justify-center bg-gray-50 dark:bg-gray-800 rounded-2xl text-gray-400 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-900/50 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                            <option.icon className="w-6 h-6" />
                                        </div>
                                        <div>
                        ...
                        <div className="mt-8 bg-gray-900 rounded-3xl p-10 text-white relative overflow-hidden">
                            ...
                            <div className="relative z-10 max-w-lg">
                                ...
                                <p className="text-gray-400 dark:text-gray-300 mb-6 font-medium leading-relaxed">Enjoy exclusive discounts, faster shipping times, and early access to deals when you join our premium status.</p>
                    ...
                    <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-indigo-500/10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <button
                        ...
                        {activeSection === 'security' && (
                            <div className="max-w-xl">
                                <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-6">Security</h2>
                                <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm">Manage your account security and authentication methods.</p>

                                <div className="space-y-6">
                                    <div className="flex items-center justify-between p-6 bg-gray-50 dark:bg-indigo-900/20 rounded-2xl transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-white dark:bg-gray-800 rounded-xl flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-sm border border-gray-100 dark:border-gray-700">
                                                <Lock size={24} />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-gray-900 dark:text-white">Change Password</h4>
                                                <p className="text-xs text-gray-500 dark:text-gray-400">Receive an email to update your access safely</p>
                                            </div>
                                        </div>
                                        <button
                                            onClick={handleResetPasswordTrigger}
                                            disabled={securityStatus === 'loading'}
                                            className="px-5 py-2.5 bg-gray-900 dark:bg-indigo-600 text-white text-xs font-bold rounded-xl hover:bg-black dark:hover:bg-indigo-700 transition-all disabled:opacity-50"
                                        >
